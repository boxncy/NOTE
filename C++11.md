...
`auto function = [capture](paramters)mutable->return-type{statement}`

`function();`

-   捕获列表 [] var /=(值传递作用域所有变量，包括this) /&var /& /this,可以多个值如:[= ，&a，&this]
-   参数列表 ()
-   修饰符 mutable 常量性，可以修改常量
-   返回类型 return-type
-   函数体 {}

auto的类型为“闭包”(closure),lambda产生一个闭包类型的临时对象（右值），lamdba并非函数指针，但是在lamdba没有捕获任何变量时，可以转化为函数指针
...
-   `for_each(iterator begin ,iterator end ,function)`

注意function是一个接受单个参数的函数指针、仿函数、lambda函数
...
for_each(nums.begin() ,nums.end() ,func1);
...
仿函数（Functor）是一个能行使函数功能的类，它是一个函数对象，它定义了一个含有operator()成员函数的对象，可以视为一个一般的函数，只不过这个函数功能是在一个类中的运算符operator()中实现。仿函数比函数指针的执行速度快，因为函数指针时通过地址调用，而仿函数是对运算符operator进行自定义来提高调用的效率。此外，仿函数比一般函数更灵活，可以同时拥有两个不同的状态实体，一般函数不具备此种功能。同时，仿函数可以作为模板参数使用，因为每个仿函数都拥有自己的类型。[[1\]](https://blog.csdn.net/toby54king/article/details/105103111)[[2\]](https://blog.csdn.net/u014386899/article/details/108590804)

仿函数通常有以下四个作用：[[1\]](https://blog.csdn.net/toby54king/article/details/105103111)
...
下面是一个使用仿函数的例子，定义一个Compare类，通过实现其operator()成员函数，使得该类可以比较一个数是否大于类的成员变量m_Number。[[3\]](https://zhuanlan.zhihu.com/p/362323211)
...
{
    Compare cmp(10);
    std::cout << cmp(11) << std::endl; // 输出0，因为11不大于10
    std::cout << cmp(9) << std::endl; // 输出1，因为9小于10
    return 0;
}
```

在C++中，仿函数不是一个函数，是一个类，这个类实现了函数的功能。如果需要实现一个功能，可以定义一个类，让其中的()函数实现这个功能。比如，如果想要实现一个求和的功能，可以定义一个Sum类，让其中的()函数实现这个功能。[[4\]](https://blog.csdn.net/yuhan61659/article/details/81356051)
